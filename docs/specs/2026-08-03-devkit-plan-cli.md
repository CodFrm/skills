...
一次写入只改动被寻址的那一行（追加类命令则只插入新行）。文件其余每一个字节保持原样，包括缩进、空行、注释、键的顺序，以及 `goal: >-` 这类折叠标量的原始换行位置。
...
| 写入后的逐行比对 | 除目标行外每一行字节相同——这是「行级替换不毁文件」这个设计决定的唯一硬证据；fixture 用本仓两份真 plan 的副本 | 无 |
