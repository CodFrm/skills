...
| Output | CI verdict | `<scenario>/report.md` and evidence |
...
Everything a verification authors and observes goes under `e2e/scratch/<scenario>/`; [`../docs/verification.md`](../docs/verification.md) chooses the form — no script, a launcher, or a full one — and owns verdicts and evidence. Run scripts with `<scratch command>`, and reuse harness isolation and the oracle; its fixtures stand in for real dependencies and need the user's authorization.
