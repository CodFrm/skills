...
Use only after both static wrap-up reviews and their fix loop pass. Dispatch it at `strong` to a fresh subagent that implemented or reviewed none of the branch. **[The rules every dispatch shares](prompts.md#what-every-dispatch-shares) apply**, and every `<>` slot must be filled.
