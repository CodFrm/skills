...
pre/final HEAD, status, optional commit, each finding and the action taken, and each deciding command/exit code/observation. An interrupted
...
Return at most 12 lines: status `complete | blocked`; receipt path; final HEAD and optional commit; finding/action summary;
