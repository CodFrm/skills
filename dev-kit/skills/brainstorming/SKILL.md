...
**The spec settles what must be true and what is out of bounds; it does not carry a checklist.** The commands, the expected output and the verdicts belong to the round that implements it — [what every round owes](../using-dev-kit/SKILL.md#what-every-round-owes-whatever-its-size) puts them into the tests it writes and into what gets handed back at delivery. Keeping a second checkable list here means the same sentence in two documents with nothing to catch them drifting apart, and it pulls the spec down into how the work will be done. **What this costs you is real**: nothing downstream is committed except the tests, so **the prose here is the only durable statement of the requirement** — say each one observably enough that whoever implements it can build the check from it without asking you.
