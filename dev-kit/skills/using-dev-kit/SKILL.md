...
| `executing-plans` | **When a `ready` plan exists**, and again at the start of any session that finds one whose `status` is not `done`. Reads the whole plan and the spec, **asks one question — dispatched subagents or inline, dispatched recommended — states the worktree decision rather than asking it**, then runs without stopping between tasks: every ready task dispatched, in parallel where their `files` are disjoint, each at the model tier its plan line names, each as a mandatory round of `test-driven-development`, each judged against a command, an exit code and an observation — and each returning **the conclusion rather than the transcript**, since a dispatch that hands back everything it read has bought no isolation at all. An implementer answers with one of four statuses, and `missing context` (a hole in the plan) is repaired and re-sent rather than counted as a failure. **With subagents, evidence holding earns a task its review, not `done`**: that commit goes to a second subagent that did not write it, read against the task's goal, the project's own conventions and the code, **and that same subagent fixes what it finds** — one TDD round per finding, its own commit, the findings themselves coming back with it. Two subagents per task, and the fixing never goes back to the implementer whose blind spot produced the finding. The `inline` path has no per-task review, and waits for wrap-up. **Only the orchestrator writes the plan file.** Wrap-up is two more dispatched reviews at once, over the whole branch — one against the spec, one on the code — then fixes, re-reviewed, to a ceiling of three rounds. Ends in a verification report whose last section is how the user reproduces it themselves — **and which finds without fixing**: a requirement that does not hold is written down as not holding and taken to the user, not repaired underneath the report. |
...
requirement / fault
  ...
  → executing-plans — a loop, not a stage: everything indented below runs once per task
       ...
       ├ first gate — evidence: a command, an exit code, an observation, the goal checked
       │   point by point. A second shortfall blocks that task rather than buying a third
       │   dispatch, and the loop carries on with whatever it does not gate
       ├ second gate, dispatched mode only — that task's commit read by a second subagent
       │   that did not write it, on three axes: the goal, the project's own conventions, the
       │   code. Scope is git show <sha>, never the working tree — a sibling may be writing
       │   into it. Inline has no such step: there, wrap-up is the first outside reading
       │  ↳ the same subagent then fixes what it found, one TDD round per finding, in its own
       │     commit — and the findings come back with it, each with the test that covers it,
       │     because a fix you cannot see is a claim. Once per task: blocking left open blocks
       │     it, anything smaller becomes a note that travels to wrap-up
  ...
  → wrap-up — two dispatched reviews at once, then fixes, to a ceiling of three rounds
       ...
       ↳ the per-task reviews do not shrink this, and are never mentioned in these prompts:
         a duplicate implementation, one concern solved two ways, an interface that does not
         line up at both ends are all invisible to a reviewer holding one commit
  ...
  → the verification report — e2e/scratch/<spec-slug>/report.md, one directory with its
      logs/ resources/ screenshots/ videos/ beside it, whatever the project looks like
       ...
       ├ it finds and does not fix: a requirement that does not hold is written down as not
       │   holding and taken to the user, never repaired underneath the report — a step that
       │   fixes as it goes has verified its own repair, over code no reviewer has read
...
- **A dispatched review before the change is called finished.** **Nothing is judged complete inside the context that produced it, and that invariant has no size exemption** — a small change reviewed by its own author is exactly the case where "it is obviously fine" goes unchallenged. Its scope is **the whole diff of this change against the branch point**, on two axes: does it do what the spec says, and is the code right. Under a plan those are [two subagents run at once](../executing-plans/SKILL.md#wrap-up-two-reviews-at-once); on a change too small for a plan they are one dispatch with two headings, the spec axis skipped in one line where there legitimately is no spec.
  ...
  **Under a plan run with subagents, every task owes one of its own as well**, [scoped to that single commit and followed by its own fix](../executing-plans/SKILL.md#the-task-review-and-its-fix-the-second-gate-before-done), before the task is marked `done` — a convention broken in task 1 is a convention five later tasks copy, correctly, because matching the code around them is what they were told to do. It is an addition to the branch-level pass and never a substitute for it, and the `inline` path does without it.
