...
Every dispatched task requires `profile`: use `write` for implementation, review-and-fix, and runtime verification; use `read-only` for investigation and static reviews. Resolve the plan's relative model tier against models available now, then pass a real `provider/model`; do not persist a tier mapping or let the package infer model strength. Use single for one task; use parallel only after the conservative independence gate in `executing-plans` has explicit plan or read-only-subagent evidence, and use chain only when the next task consumes `{previous}`. A task's explicit `tools` must stay within its profile and must not include `subagent`.

The main session owns task selection, routing, decisions from subagent reports, mechanical commit/SHA checks, verification acceptance, and every plan state write. In `subagent` mode it does not review source code, commits or diffs; implementation, batch review, static review and code-level verification stay with subagents. A child completion is a report, not permission to change `.dev-kit/plans/*.yaml`.
