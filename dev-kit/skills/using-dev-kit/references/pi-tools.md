...
Every dispatched task requires `profile`: use `write` for implementation, review-and-fix, and runtime verification; use `read-only` for investigation and static reviews. Resolve the plan's relative model tier against models available now, then pass a real `provider/model`; do not persist a tier mapping or let the package infer model strength. Use single for one task, parallel only for ready tasks whose `files` do not overlap, and chain only when the next task consumes `{previous}`. A task's explicit `tools` must stay within its profile and must not include `subagent`.

The main session still owns task selection, evidence judgement, commits, reviews, verification, and every plan state write. A child completion is a report, not permission to change `.dev-kit/plans/*.yaml`.
