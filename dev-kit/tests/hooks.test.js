...
test('JSON escaping survives quotes, backslashes, tabs and carriage returns', () => {
  // An escaping bug does not corrupt the output, it truncates at the first offending character —
  // often into JSON that still parses. Against a written fixture rather than the real SKILL.md:
  // whether the bootstrap happens to be worded with a quote in it must not decide whether the
  // escaping is covered.
  const fake = path.join(tmp, 'escaping', 'hooks')
  fs.mkdirSync(fake, { recursive: true })
  fs.copyFileSync(SESSION_START, path.join(fake, 'session-start'))
  const skillDir = path.join(tmp, 'escaping', 'skills', 'using-dev-kit')
  fs.mkdirSync(skillDir, { recursive: true })
  const body = 'quote " backslash \\ both "\\"\ntab\there\ncarriage return\r\ntrailing text\n'
  fs.writeFileSync(path.join(skillDir, 'SKILL.md'), body)

  const r = run(path.join(fake, 'session-start'), { pluginRoot: path.join(tmp, 'escaping') })
  const ctx = JSON.parse(r.out).hookSpecificOutput.additionalContext
  assert.ok(ctx.includes('quote " backslash \\ both "\\"'), 'quotes and backslashes must survive')
  assert.ok(ctx.includes('tab\there'), 'a tab must survive')
  assert.ok(ctx.includes('trailing text'), 'nothing after an escaped character may be dropped')
})
