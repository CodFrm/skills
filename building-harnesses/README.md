...
- **五件套合同**：接入已有闸门、精确作用域 + 合法实现豁免、报错信息指明正解、加载真实配置的双向守护测试、文档回指 harness 且落地时全绿（存量太多修不完时用**棘轮**：圈定豁免基线只减不增，新代码立即达标）。
...
- **整套流程 bootstrap（三层）**：
  - *原则层*：AGENTS.md 瘦身为入口页（CLAUDE.md 仅导入），原则含「确认 bug 存在→失败测试→修复」（复现不了就明说并停手）、TDD/BDD 先行（行为化标题；测试挂了改代码不改测试、删无意义测试）、修根因禁 `as any`/吞异常、SOLID 落到本仓缝隙并标注原则字母（「注册扩展、共享代码禁 type 分支」即具体化的 OCP）、**边界校验+禁无意义兜底**（不可能发生的 case 加守卫只会藏住真 bug）、直接替换不加适配层、**复用先于新造**（写之前先 grep；第二次出现才提取；同一修复出现在两处近似代码块＝第二处就是 bug）、范围纪律（光标下的过期注释/说谎文档行属界内，顺手重构属界外）、注释只写非显然的为什么、无死代码；每条标注「已由门禁强制/仅评审」并配一行 why。
  ...
  - *验证层*：`docs/verification.md` 负责「怎么确认真的能用」——廉价信号先行、一次性 scratch 脚本驱动真实应用（≠扩充永久 E2E 套件）、证据本地留档不入库、复现=确认 bug 存在那一步随后必须升级为失败的提交测试；无法直接驱动的界面（桌面 GUI）改为观察副作用（结构化日志/数据库/审计记录/headless CLI 孪生）。
...
和一个 Go 桌面应用仓库的 archtest 实践（测试套件内的 import 禁令表 + 豁免基线棘轮只减不增 + checker 自身双向守护测试）。
